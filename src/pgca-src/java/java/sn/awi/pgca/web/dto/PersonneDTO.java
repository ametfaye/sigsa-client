...
public class PersonneDTO implements Serializable {
	...
	public List<String> incorrectAttributeLight() {
		List<String> sb = new ArrayList<String>();
		if (!UtilString.isCorrect(nom))
			sb.add("Nom");
		if (!UtilString.isCorrect(prenom))
			sb.add("Prénom");
		if (!UtilString.isCorrect(civilite))
			sb.add("Civilité");
		return sb;
	}
	...
	public List<String> incorrectAttribute() {
		...
		if (!UtilString.isCorrect(prenom))
			sb.add("Prénom");
		if (!UtilString.isCorrect(strDateNais))
			sb.add("Date de naissance");
		if (!UtilString.isCorrect(civilite))
			sb.add("Civilité");
		if (!UtilString.isCorrect(nationalite))
			sb.add("Nationalité");
		if (!UtilString.isCorrect(lieunaissance))
			sb.add("Lieu naissance");
		return sb;
	}
